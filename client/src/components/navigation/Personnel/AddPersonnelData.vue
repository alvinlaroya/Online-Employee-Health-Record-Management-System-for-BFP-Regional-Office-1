        ...
        <v-form v-model="valid" lazy-validation ref="form">
          <v-card-text>
            <v-container fluid>
              ...
              <v-row class="justify-center align-center">
                <v-col sm="2" style="">
                  <div class="form-group">
                    ...
                    <input type="file" accept="image/*" @change="previewImage" style="color: black" />
              ...
              <v-row class="mt-8">
                <v-col cols="12" md="4">
                  <v-text-field label="Account Number" v-model="personnels.accountNo" dense
                    @keypress="keyPressAccountNo($event)"></v-text-field>
                </v-col>
                <v-col cols="12" md="4">
                  <v-select label="Rank" v-model="personnels.rank" :items="['FDir',
                    'FCSupt',
                    'FSSupt',
                    'FSupt',
                    'FCInsp',
                ...
                <v-col>
                  <v-select label="Ext Name" v-model="personnels.extName" :items="['Sr', 'Jr', 'I', 'II', 'III  ']"
                    dense></v-select>
                </v-col>
              </v-row>
              <!-- row2 -->
              ...
              <v-row>
                <v-col cols="12" md="5">
                  <v-text-field label="Mobile" v-model="personnels.mobile" prefix="+639" dense
                    @keypress="validateNumber($event)"></v-text-field>
                </v-col>
                <v-col cols="12" md="7">
                  <v-select label="Unit Assignment" v-model="personnels.unit" :items="[
                    'OARD - R1',
                    'OPFM - ILOCOS NORTE',
                    'OPFM - ILOCOS SUR',
                    'OPFM - La Union',
              ...
              <v-row>
                <v-col cols="12">
                  <v-text-field label="Designation" v-model="personnels.designation" dense></v-text-field>
                </v-col>
              </v-row>
              <v-row>
                <v-col cols="12">
                  <v-text-field label="Address" v-model="personnels.address" dense></v-text-field>
                </v-col>
              </v-row>
              <!-- row5 -->
              <v-row>
                <v-col cols="12" md="4">
                  <v-select label="Civil Status" v-model="personnels.civilStatus"
                    :items="['Single', 'Married', 'Widowed', 'Separated']" dense></v-select>
                </v-col>
                <v-col cols="12" md="4">
                  <v-select label="Gender" v-model="personnels.gender" :items="['Male', 'Female']" dense></v-select>
                </v-col>
                <v-col cols="12" md="4">
                  <v-menu ref="menu" v-model="menu" :close-on-content-click="false" :return-value.sync="date"
                    transition="scale-transition" offset-y min-width="auto">
                    <template v-slot:activator="{ on, attrs }">
                      <v-text-field v-model="personnels.dateOfBirth" label="Date of Birth" prepend-icon="mdi-calendar"
                    ...
                    <v-date-picker v-model="personnels.dateOfBirth" no-title scrollable>
                      ...
                      <v-btn text color="primary" @click="$refs.menu.save(personnels.dateOfBirth)">
                        OK
                      </v-btn>
                    </v-date-picker>
                  </v-menu>
              ...
              <v-row>
                <v-col cols="12" md="4">
                  <v-text-field label="Philhealth" v-mask="'##-#########-#'" v-model="personnels.philhealth"
                    dense></v-text-field>
                </v-col>
                <v-col cols="12" md="8">
                  <v-text-field label="Remarks" v-model="personnels.remarks" dense></v-text-field>
                </v-col>
              </v-row>
              <v-row>
                <v-col cols="12">
                  <v-text-field label="Place Of Birth" v-model="personnels.placeOfBirth" dense></v-text-field>
                </v-col>
              </v-row>
              <v-row>
                <v-col cols="12">
                  <v-text-field label="Occupation" v-model="personnels.occupation" dense></v-text-field>
                </v-col>
              </v-row>
            </v-container>
          </v-card-text>
